...
//                        if(key.equals("the_shape")||key.equals("the_point")){
//                            column.setCellFactory(TextFieldTableCell.forTableColumn());
//                            column.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<Map<String, SimpleStringProperty>, String>, ObservableValue<String>>() {
//                                @Override
//                                public ObservableValue<String> call(TableColumn.CellDataFeatures<Map<String, SimpleStringProperty>, String> param) {
//                                    String geoJSON = param.getValue().get(key).getValue();
//                                    if (geoJSON.contains("="))
//                                        geoJSON = geoJSON.replace("=", ":");
//                                    JSONObject jsonObject = new JSONObject(geoJSON);
//                                    return new SimpleStringProperty(jsonToWkt(jsonObject.toString()));
//                                }
//                            });
//                        }
//                        else{
...
//                        map2.put(string, new SimpleStringProperty(result.get(i).get(string).toString()));
        ...
        btnShowWkt.setOnMouseClicked(event -> {
            if (tvDocs.getItems().size() != 0) {
                String field = "";
                for (int i = 0; i < tvDocs.getColumns().size(); i++) {
                    if (tvDocs.getColumns().get(i).getText().equals("the_shape")) {
                        field = "the_shape";
                    }
                    if (tvDocs.getColumns().get(i).getText().equals("the_point")) {
                        field = "the_point";
                    }
                }
                if (isWkt(tvDocs.getItems().get(0).get(field).getValue())) {
                    new Alert(Alert.AlertType.INFORMATION, "表中内容已经是wkt格式！", ButtonType.OK)
                            .showAndWait();
                } else {
                    int tag = 0;
                    for (int i = 0; i < tvDocs.getColumns().size(); i++) {
                    ...
                    if (tag != 0) {
                        if (tag == 1) {
                            for (int j = 0; j < tvDocs.getItems().size(); j++) {
                                String geoJSON = tvDocs.getItems().get(j).get("the_shape").get();
                                if (geoJSON.contains("="))
                                    geoJSON = geoJSON.replace("=", ":");
                                JSONObject jsonObject = new JSONObject(geoJSON);
                                tvDocs.getItems().get(j).get("the_shape").set(jsonToWkt(jsonObject.toString()));
                            }
                        }
                        if (tag == 2) {
                            for (int j = 0; j < tvDocs.getItems().size(); j++) {
                                String geoJSON = tvDocs.getItems().get(j).get("the_point").get();
                                if (geoJSON.contains("="))
                                    geoJSON = geoJSON.replace("=", ":");
                                JSONObject jsonObject = new JSONObject(geoJSON);
                                tvDocs.getItems().get(j).get("the_point").set(jsonToWkt(jsonObject.toString()));
                            }
                        }
                    }
                }
            } else {
                new Alert(Alert.AlertType.INFORMATION, "表格中无内容", ButtonType.OK)
                        .showAndWait();
            }
        });
...
//            if (tvDocs.getColumns().size() == 1) {
            if (tvDocs.getItems().size() <= 0) {
                ...
                if (!isWkt(tvDocs.getItems().get(0).get(field).getValue())) {
                    new Alert(Alert.AlertType.INFORMATION, "请点击按钮，以wkt格式显示数据！", ButtonType.OK)
                            .showAndWait();
                } else {
                    //在表格最后一行加入数据
                    ...
                    for (int i = 0; i < tvDocs.getColumns().size(); i++) {
                        ...
                        else if (tvDocs.getColumns().get(i).getText().equals("the_shape") || tvDocs.getColumns().get(i).getText().equals("the_point")) {
                            docAdd.put(tvDocs.getColumns().get(i).getText(), new SimpleStringProperty("填入wkt数据"));
                        } else
...
//                Boolean bool = true;
                ...
                for (int i = tvDocs.getItems().size() - addNumber; i < tvDocs.getItems().size() - addNumber + addValidNumber; i++) {
                    if (tvDocs.getItems().get(i).containsKey("the_shape")) {
                        if (tvDocs.getItems().get(i).get("the_shape").getValue().equals("填入wkt数据"))
                            bool = false;
                    }
                    if (tvDocs.getItems().get(i).containsKey("the_point")) {
                        if (tvDocs.getItems().get(i).get("the_point").getValue().equals("填入wkt数据"))
                            bool = false;
                    }
                }
                if (!bool) {
...
//                                mapAdd.put(string, map2);
                                //表格内添加wkt格式数据
                                String wkt = map.get(string).getValue();
...
//                    }
        ...
        btnSaveModifiedDocs.setOnAction(event -> {
            if (tvDocs.getItems().size() <= 0) {
                new Alert(Alert.AlertType.INFORMATION, "表中无数据", ButtonType.OK)
                        .showAndWait();
            } else {
                String field = "";
                for (int i = 0; i < tvDocs.getColumns().size(); i++) {
                        ...
                        if (!mapModify.get("lsid").getValue().equals("可以编辑的行")) {
                            ...
                            for (String string : mapModify.keySet()) {
                                if (string.equals("the_shape")) {
                                    ...
                                    String wkt = mapModify.get(string).getValue();
                                    String geoJSON = wktToJson(wkt);
